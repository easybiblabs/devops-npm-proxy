...
try {
  ...
  /* I have to do this for reasons */
  env_populate(config);
  function env_populate(object,stack) {
    for (var property in object) {
      if (object.hasOwnProperty(property)) {
        if(typeof object[property] === "object") {
          env_populate(object[property], stack + '.' + property);
        } else if(  typeof(object[property]) === 'string' &&
                    object[property].startsWith('process.env.')) {
          env_str = object[property].slice(12);
          object[property] = process.env[env_str];
        }
      }
    }
  }
